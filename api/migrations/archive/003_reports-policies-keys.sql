-- Migration 003: Reports, policies, and registered keys tables.
--
-- Creates:
--   reports            - Compliance report records
--   report_coverage    - Coverage trend data points
--   policies           - Governance policy definitions
--   policy_triggers    - Policy trigger history
--   registered_keys    - Registered LLM API keys
--
-- All configuration/report tables include project_id for multi-tenant scoping.
-- UNIQUE constraint on registered_keys.fingerprint prevents duplicate key registration.
...
-- Registered LLM API keys
-- UNIQUE constraint on fingerprint prevents duplicate key registration at the DB level.

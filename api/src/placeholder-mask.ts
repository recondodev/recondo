 ...
 * (`api/tests/placeholder-mask.test.ts`) asserts the Rust and TS
 * consumers read equal sets from the JSON.
 */
...
// Locate shared/placeholder-prefixes.json relative to this source file.
 ...
 * this module so it works whether the module is imported from
 * `src/resolvers/` or from `tests/`. At test time the file is read
 * from the repo root; at runtime the file is copied into the
 * container image alongside the JS bundle.
 */
function loadSharedJson(): {
  prefixes: string[];
  replacement: string;
} {
  // Walk up to find the shared/ directory — the module can be invoked
  // from `src/placeholder-mask.ts` (dev) or `dist/src/placeholder-mask.js`
  // (prod build).
 ...
 * containing `]`.
 */
export function placeholderLikePatterns(): readonly string[] {
  return PLACEHOLDER_PREFIXES.map((p) => `%${p} source: %`);
}

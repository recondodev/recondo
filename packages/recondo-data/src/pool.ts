...
export function getPool(): pg.Pool {
  if (!pool) {
    ...
    // at PID 1).
    //
    // NOTE: env var name is RECONDO_API_APP_NAME_SUFFIX (not
    // RECONDO_DATA_APP_NAME_SUFFIX) — sibling test runners depend
    // on this exact name. DO NOT rename when moving the file.
    const appNameSuffix =
...
export async function closePool(): Promise<void> {
  // C2 D-P3 fix: reset the module-level singleton BEFORE awaiting
  // pool.end(), so concurrent getPool() calls during teardown don't
  // observe a closed pool. The previous implementation awaited end()
  // first and reset to null after, which left a window where a
  // racing getPool() returned the closed pool and subsequent queries
  // failed with "pool ended". Idempotent: calling closePool twice is
